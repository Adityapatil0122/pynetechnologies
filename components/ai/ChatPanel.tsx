...
import { pynAiSuggestions, type PynAiMessage } from "@/lib/pyn-ai";

const greeting: PynAiMessage = {
  role: "assistant",
  content: "Hi, I'm PYN Guide. Ask me about services, automation, products, pricing, or how to start a project."
};

export function ChatPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [messages, setMessages] = useState<PynAiMessage[]>(() => {
    if (typeof window === "undefined") return [greeting];

    try {
      const saved = window.localStorage.getItem("pyn-ai-messages");
      if (!saved) return [greeting];
      const parsed = JSON.parse(saved) as PynAiMessage[];
  ...
  useEffect(() => {
    window.localStorage.setItem("pyn-ai-messages", JSON.stringify(messages.slice(-16)));
  ...
  async function sendMessage(text: string) {
    ...
    const nextMessages: PynAiMessage[] = [...messages, { role: "user", content: clean }];
    setMessages(nextMessages);
    setInput("");
    setLoading(true);

    try {
      const response = await fetch("/api/pyn-ai", {
        method: "POST",
      ...
      setMessages((current) => [
        ...
        {
          role: "assistant",
          content: data.reply || "I did not understand that. Ask me about PYN services, automation, apps, websites, or contact details."
        }
      ]);
    } catch {
      setMessages((current) => [
        ...
        {
          role: "assistant",
          content: "I'm having trouble connecting right now, but PYN can help with websites, apps, WhatsApp automation, design, and marketing."
        }
      ]);
    } finally {
      setLoading(false);
  ...
  function resetChat() {
    ...
    setInput("");
    window.localStorage.removeItem("pyn-ai-messages");
  }
      ...
      {open ? (
        <>
          ...
          <motion.aside
            ...
            aria-label="PYN Guide chat panel"
          >
            <div className="flex items-center justify-between gap-4 border-b border-[rgba(30,34,51,0.08)] bg-[#f5fbff] p-5">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <h2 className="truncate text-lg font-black">PYN Guide</h2>
                  <p className="truncate text-sm font-bold text-[var(--muted)]">PYN support guide &middot; quick replies</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button className="focus-ring rounded-full p-2 transition hover:bg-white" type="button" onClick={resetChat} aria-label="Reset PYN Guide chat">
                  <RotateCcw className="h-5 w-5" />
                </button>
                <button className="focus-ring rounded-full p-2 transition hover:bg-white" type="button" onClick={onClose} aria-label="Close PYN Guide">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
            ...
            <div ref={scrollRef} className="grid flex-1 gap-4 overflow-y-auto p-5">
              ...
              {loading ? (
                <div className="flex items-center gap-3 text-sm font-black text-[var(--muted)]">
                  ...
                  PYN Guide is thinking
                </div>
              ) : null}
            </div>

            <div className="grid gap-4 border-t border-[rgba(30,34,51,0.08)] p-5">
              {messages.length <= 2 ? <SuggestedPrompts prompts={pynAiSuggestions} onPick={sendMessage} disabled={loading} /> : null}
              <form onSubmit={handleSubmit} className="flex items-end gap-3">
                ...
                <button
                  ...
                  aria-label="Send message to PYN Guide"
                >
                  <ArrowUp className="h-5 w-5" />
                </button>
              </form>
              <p className="text-center text-[11px] font-semibold leading-4 text-[var(--muted)]">
                Please confirm important project details with the PYN team.
              </p>
            </div>
          </motion.aside>
        </>
